...
def update_database(df_freq):
    level_of_theory = "CCSD(T)"
    methods = ["physnet_MP2_NN1","physnet_MP2_NN2", "mp2", "physnet_CCSD(T)_NN1","physnet_CCSD(T)_NN2","CCSD(T)"]
    df = pd.read_csv("/mlatom/home/fatemealavi/anharmonic/ANI_1ccx_CH3OH_new_model/CH3OH_ANI_1ccx_TLOnCCSDT_SP_energyShifter_FixL1and3/freq_CH3OH_by_TL_ani_1ccx/harmfreq_otherMethods.csv", sep = ",", header = 0)
    for meth in methods:    
